...
Beginning in v0.3A, the actor and organization are derived from a validated access token and an active database user. Analyst and admin roles may record decisions; viewers remain read-only. The decision service contract and append-only lineage are unchanged.
